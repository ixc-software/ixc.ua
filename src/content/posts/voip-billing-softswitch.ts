...
const zh = `准确的 **VoIP 计费**把已路由分钟转化为收入。在**软交换**上，每条 **CDR（呼叫详单）**都进入评级、余额与财务导出。

## 计费如何工作

呼叫结束后，交换机记录 CLI/CLD、时长、拆线原因、客户与供应商以及所应用费率。计费引擎对 CDR 评级，并更新预付或后付余额。

## 预付与后付

**预付** — 呼叫完成前或完成时余额必须覆盖该呼叫；常见于批发商、电话卡与高风险账户。

**后付** — 按周期从汇总 CDR 开票；常见于带信用额度的签约运营商。

许多平台以共享路由与报表**同时支持两者**。

## 评级要点

- 按前缀的费率表，含生效日期与计费增量（6+6、1+1）
- 按客户或产品线的资费计划
- 与商务条款匹配的舍入规则
- 自动导入供应商费率，避免人工滞后
...
软交换上的计费应实时、前缀准确，并对财务与 NOC 可见——尤其在预付敞口较高时。`;
...
export const voipBillingPost: BlogPost = {
  ...
  zh: {
    title: '软交换上的 VoIP 计费：预付、后付与实时 CDR',
    description:
      'VoIP 软交换计费如何工作：CDR 评级、预付与后付、费率表、实时余额，以及避免收入泄漏。',
    excerpt:
      '基于 CDR 的计费、预付与后付模式、费率管理，以及运营商在 VoIP 软交换上需要的报表。',
    content: zh,
    faq: [
      {
        q: 'VoIP 中的 CDR 是什么？',
        a: '呼叫详单记录每通呼叫的号码、时长、费率与拆线原因——是计费与利润分析的基础。'
      },
      {
        q: '一台软交换能否同时处理预付和后付？',
        a: '可以。许多平台以共享路由、余额与报表同时支持两种模式。'
      },
      {
        q: '预付余额应多快更新？',
        a: '理想情况下在呼叫完成前或完成时实时更新，以限制信用敞口。'
      }
    ]
  }
};
